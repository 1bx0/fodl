...
import { AaveLendingAdapter, CompoundForksLendingAdapter, CompoundPriceOracleMock, FodlNFT } from '../typechain'
...
describe('SimplePositionStopLossConnector', () => {
  ...
  let unconfiguredAccount: AllConnectors
  let fodlNFT: FodlNFT
  ...
  const openSimplePositionWithoutLeverage = async (eoa = alice) => {
    const supplyAmount = ONE_ETH
    const price = await getAavePrice(AAVE_PLATFORM, WETH, DAI)
    const borrowAmount = ONE_ETH.mul(price).div(MANTISSA).div(10)
  ...
  describe('configureStopLoss()', () => {
    describe('when position has not been opened yet', async () => {
      it('reverts', async () => {
        await expect(
          account.configureStopLoss(MAX_UNWIND_FACTOR_BN, MAX_SLIPPAGE_INCENTIVE_BN, MANTISSA)
    ...
    describe('when position is open but configured limit is over current collateral use', () => {
      it('reverts', async () => {
        ...
        await expect(
          account.configureStopLoss(MAX_UNWIND_FACTOR_BN, MAX_SLIPPAGE_INCENTIVE_BN, currentCollateralUsage)
        ).to.be.revertedWith('SLC3')
      })
    })
    ...
    describe('when position is open', async () => {
      ...
      it('allows to change stop loss parameters', async () => {
        await account.configureStopLoss(MAX_UNWIND_FACTOR_BN, MAX_SLIPPAGE_INCENTIVE_BN, MANTISSA)
      ...
      it('reverts if unwindFactor is over 100% (1e18)', async () => {
        await expect(
          account.configureStopLoss(MAX_UNWIND_FACTOR_BN.add(1), MAX_SLIPPAGE_INCENTIVE_BN, MANTISSA)
        ).to.be.revertedWith('SLC1')
      })
      it('reverts if slippageIncentive is over 100% (1e18)', async () => {
        await expect(
          account.configureStopLoss(MAX_UNWIND_FACTOR_BN, MAX_SLIPPAGE_INCENTIVE_BN.add(1), MANTISSA)
        ).to.be.revertedWith('SLC2')
      })
      it('reverts if collateralUsageLimit is over 100% (1e18)', async () => {
        await expect(
          account.configureStopLoss(MAX_UNWIND_FACTOR_BN, MAX_SLIPPAGE_INCENTIVE_BN, MANTISSA.add(1))
        ).to.be.revertedWith('SLC3')
      })

      it('reverts if called by non owner', async () => {
        await expect(
          account.connect(mallory).configureStopLoss(MAX_UNWIND_FACTOR_BN, MAX_SLIPPAGE_INCENTIVE_BN, MANTISSA)
        ).to.be.revertedWith('FA2')

        await expect(
          account.connect(bot).configureStopLoss(MAX_UNWIND_FACTOR_BN, MAX_SLIPPAGE_INCENTIVE_BN, MANTISSA)
        ).to.be.revertedWith('FA2')

        await fodlNFT.connect(alice).transferFrom(alice.address, bot.address, account.address)

        await expect(account.connect(bot).configureStopLoss(MAX_UNWIND_FACTOR_BN, MAX_SLIPPAGE_INCENTIVE_BN, MANTISSA))
          .to.not.be.reverted
      })
    })
  })
  ...
  describe('executeStopLoss()', () => {
    describe('reverts', () => {
      it('when position is not yet defined', async () => {
        await expect(unconfiguredAccount.connect(bot).executeStopLoss()).to.be.revertedWith('SLC8')
      })

      it('reverts if unwind factor is not configured', async () => {
        await openSimplePositionWithoutLeverage()
        await expect(account.connect(bot).executeStopLoss()).to.be.revertedWith('SLC7')
      })
    })
